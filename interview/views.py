...
def about_view(request):
    techs = [
        {"name": "Django", "icon": "fas fa-layer-group", "color": "#092e20"},
        {"name": "Python", "icon": "fab fa-python", "color": "#3776ab"},
        {"name": "PostgreSQL", "icon": "fas fa-database", "color": "#336791"},
        {"name": "Groq LLM", "icon": "fas fa-brain", "color": "#7209b7"},
        {"name": "Bootstrap", "icon": "fab fa-bootstrap", "color": "#7952b3"},
        {"name": "Render", "icon": "fas fa-cloud", "color": "#4361ee"},
    ]
    return render(request, "interview/about.html", {"techs": techs})


@require_GET
def api_questions(request, role_id):
    role = get_object_or_404(Role, id=role_id)
    from .ai_engine import ai_generate_questions

    questions = ai_generate_questions(role.name, count=15)
    if not questions:
        # fallback to database
        questions = list(
            Question.objects.filter(role_id=role_id).values(
                "id", "text", "difficulty", "keywords"
            )
        )
    else:
        # Delete old AI questions for this role and save fresh ones
        Question.objects.filter(role=role).delete()
        saved = []
        for q in questions:
            obj = Question.objects.create(
                ...
                keywords=q.get("keywords", []) + ["ai_generated"],
            )
            saved.append(
                {
                    "id": obj.id,
...
def api_answer(request):
    ...
    if getattr(settings, "USE_AI", False):
        try:
            ...
            feedback_parts = []
            feedback_parts.extend(result.get("strengths", []))
            feedback_parts.extend(result.get("weaknesses", []))
            feedback_parts.extend(result.get("improvements", []))
            feedback = "\n".join(feedback_parts)
            if not feedback.strip():
                feedback = "No feedback provided."
        except Exception:
            score, feedback = score_answer(answer_text, question.keywords or [])
    else:
        score, feedback = score_answer(answer_text, question.keywords or [])
...
def api_answer_browsable(request):
    ...
    if request.method == "GET":
        ...
        sample_questions = list(
            Question.objects.select_related("role").values(
        ...
        user_session = serializer.validated_data.get(
            "user_session"
        ...
        try:
            ...
            return Response(
                {"error": "Invalid question_id"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def api_start_session(request):
    ...
    session = Session.objects.create(user_session=secrets.token_hex(8), role=role)
    return JsonResponse(
        {
            "session_id": session.id,
            "user_session": session.user_session,
